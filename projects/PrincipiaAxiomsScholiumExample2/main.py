...
class TwoSwingingBalls(Scene):
    ...
    def construct(self):
        ...
        line_EF    = Line()
        line_EF.put_start_and_end_on(dot_E.get_center(),dot_F.get_center())
        line_EF.set_color(RED)
        self.play(Write(line_EF))

        line_GH    = Line()
        line_GH.put_start_and_end_on(dot_G.get_center(),dot_H.get_center())
        line_GH.set_color(BLUE)
        self.play(Write(line_GH))

        dot_C  = Dot(ax.coords_to_point(4.5,7), color=RED)
        dot_D  = Dot(ax.coords_to_point(5.5,6.7), color=BLUE)
